...
First initialize the Federated Connection Authorizer:
...
- Learn about [authorization patterns](/unified-docs/deployment/authorization) for additional security options
- Explore [tool calling](/unified-docs/tool-calling) to understand how tools work in Genkit
- Check out [interrupts](/unified-docs/interrupts) for handling user interactions in flows
- See [RAG](/unified-docs/rag) for building retrieval-augmented generation applications
</LanguageContent>

<LanguageContent lang="go python">
The Auth0 AI plugin is currently only available for JavaScript. For Go and Python applications, consider using:

- Standard OAuth 2.0 flows with your preferred authentication library
- Direct integration with Auth0's APIs
- Custom authorization middleware for your specific use case

See the [authorization patterns](/unified-docs/deployment/authorization) guide for language-agnostic security approaches.
</LanguageContent>