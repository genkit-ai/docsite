...
JavaScript provides a comprehensive `context` object that can propagate generation and execution context throughout the process. This context is made available to all actions including [flows](/unified-docs/flows), [tools](/unified-docs/tool-calling), and [prompts](/unified-docs/dotprompt).
...
- Learn about [creating flows](/unified-docs/flows) to build workflows that use context
- Explore [tool calling](/unified-docs/tool-calling) to understand how context propagates to tools
- See [developer tools](/unified-docs/devtools) for debugging context-aware applications
- Check out [generating content](/unified-docs/models) for using context in generation calls
