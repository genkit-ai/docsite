...
If you already build UIs with the [Vercel AI SDK UI](https://ai-sdk.dev/docs/ai-sdk-ui) library, the `@genkit-ai/vercel-ai` package provides an adaptor for its `useChat` hook. The `GenkitChatTransport` adaptor connects `useChat` to your agent, so you can assemble the interface from Vercel's [AI Elements](https://elements.ai-sdk.dev/) components while getting all the benefits of Genkit on the backend.
