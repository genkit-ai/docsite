...
Genkit solves this with the [**Agents API**](https://genkit.dev/docs/agents/overview/), which packages all of that behind one conversational interface. You define an agent on the server, then drive it with the same `chat()` API whether it runs in process or behind an HTTP endpoint.
...
We are launching a JavaScript client, so a web frontend can talk to the same agent endpoint.
