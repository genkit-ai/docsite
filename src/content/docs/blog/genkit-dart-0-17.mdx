...
## Export OpenTelemetry GenAI traces and use curated OpenAI models
...
Alongside the telemetry release, `genkit_openai` now ships with a curated catalog of capability presets that resolves automatically when you call `openAI.model` with a model ID (or via typed shortcuts on `OpenAIModels`):
