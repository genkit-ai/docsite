...
date: 2026-09-21
authors: chris-gill
description: Genkit Dart 0.17 brings resumable generate calls and agent snapshots, background subagents, pluggable telemetry with genkit_otel, and an updated OpenAI model catalog.
cover:
...
As we move toward a stable 1.0 release, [Genkit Dart](https://genkit.dev/docs/dart/overview/) 0.17 brings resumable generation and agent snapshots, background subagents, pluggable OpenTelemetry instrumentation, and an updated OpenAI model catalog. Failed or aborted `generate` calls and agent turns now preserve completed tool rounds so you can resume where execution stopped, and orchestrator agents can delegate long-running tasks to background subagents without blocking the main conversation. This release also keeps core free of OpenTelemetry dependencies via the new `genkit_otel` package, adds preconfigured capability presets in `genkit_openai`, and upgrades `genkit_mcp`, `genkit_anthropic`, and `genkit_google_genai`.
...
## Updated OpenAI model catalog

The `genkit_openai` plugin now ships with an updated catalog of chat and embedding models with preconfigured capability presets for vision, tool calling, and structured outputs. Capabilities resolve automatically when you pass a model ID to `openAI.model`, or you can use the typed shortcuts on `OpenAIModels` and `OpenAIEmbedders` for IDE autocomplete:
