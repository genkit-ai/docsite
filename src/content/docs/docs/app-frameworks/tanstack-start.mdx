...
export const corsCallout = (
  <Aside type="caution" title="Enable CORS on the backend">
    Your TanStack Start dev server runs on <code>http://localhost:3000</code>{' '}
    and your Genkit backend runs on a different port, so the browser will block
...
<video
  ...
  aria-label="Preview of the Bargain Chef app that you will build in this TanStack Start tutorial"
>
  ...
  Chef app that you will build in this TanStack Start tutorial.
</video>

<Lang lang="js">

You can find the [finished code on GitHub](https://github.com/genkit-ai/samples/tree/main/quickstarts/app-frameworks/tanstack-start/api-route).
...
<Aside type="note" title="Already have a standalone Genkit backend?">
  Follow this tutorial for the TanStack Start UI structure, but point{' '}
  <code>streamFlow</code> at your existing backend route instead of the API
  route. See{' '}
...
You can find the [finished code on GitHub](https://github.com/genkit-ai/samples/tree/main/quickstarts/app-frameworks/tanstack-start/standalone).
...
```ts title="src/routes/api/bargainChefFlow/$.ts"
...
Replace the contents of the index route with the following. The `FLOW_URL` points to your backend, so adjust the port or route if your backend doesn't run at `http://localhost:8080/bargainChefFlow`.

<Aside type="note" title="Where is the index route?">
  Recent versions of the TanStack CLI scaffold routes under `src/routes/`, so
  the index route is `src/routes/index.tsx`. Older versions used
  `app/routes/index.tsx`. Use whichever path your generated project contains.
</Aside>
...
```css title="src/routes/index.css"
:root {
  font-family:
