...
You can find the [finished code on GitHub](https://github.com/genkit-ai/samples/tree/main/quickstarts/app-frameworks/tanstack-start).
...
TanStack Start server routes receive a standard Web `Request` object, so `fetchHandlers` plugs in directly without any adapter. The route's `server.handlers` map exposes the flow over HTTP, and the splat (`$`) segment catches the flow's streaming subpath so the same handler serves both the initial POST and streamed chunks.
