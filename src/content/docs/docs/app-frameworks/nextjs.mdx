...
<video
  ...
  aria-label="Preview of the Bargain Chef app that you will build in this Next.js tutorial"
>
  ...
  Chef app that you will build in this Next.js tutorial.
</video>

You can find the [finished code on GitHub](https://github.com/genkit-ai/samples/tree/main/quickstarts/app-frameworks/nextjs).
...
<Aside type="note" title="Already have a standalone Genkit backend?">
  Follow this tutorial for the Next.js UI structure, but point{' '}
  <code>streamFlow</code> at your existing backend route instead of the
  in-project route. See{' '}
  <a href="#use-a-standalone-backend-instead">
...
import type { BargainChefInput, PartialRecipe } from '@/genkit/bargainChefFlow';
...
Replace the contents of `src/pages/index.tsx` with the following. The Pages Router imports `streamFlow` from the generic `genkit/beta/client` because the `@genkit-ai/next` integration is App Router only. The component imports the flow's TypeScript types with `import type`, so the UI and backend stay in sync without adding server code to the browser bundle:
...
>
  ...
  run a Genkit flow in the Developer UI, showing the bargainChefFlow with input
  parameters and the resulting structured output.
</video>
