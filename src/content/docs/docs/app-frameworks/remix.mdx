...
<video
  ...
  aria-label="Preview of the Bargain Chef app that you will build in this Remix tutorial"
>
  ...
  Chef app that you will build in this Remix tutorial.
</video>

<Lang lang="js">

You can find the [finished code on GitHub](https://github.com/genkit-ai/samples/tree/main/quickstarts/app-frameworks/remix/server-route).
...
<Aside type="note" title="Already have a standalone Genkit backend?">
  Follow this tutorial for the Remix UI structure, but point{' '}
  <code>streamFlow</code> at your existing backend route instead of the resource
  route. See{' '}
...
You can find the [finished code on GitHub](https://github.com/genkit-ai/samples/tree/main/quickstarts/app-frameworks/remix/standalone).
...
Replace the contents of `app/routes/home.tsx` with the following. The `FLOW_URL` points to your backend, so adjust the port or route if your backend doesn't run at `http://localhost:8080/bargainChefFlow`.
