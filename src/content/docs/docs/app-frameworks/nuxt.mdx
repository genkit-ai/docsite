...
<Aside type="note" title="Already have a standalone Genkit backend?">
  Follow this tutorial for the Nuxt UI structure, but point <code>streamFlow</code> at your existing backend route instead of the Nitro server route. See <a href="#use-a-standalone-backend-instead">Use a standalone backend instead</a> for the small changes required.
</Aside>
