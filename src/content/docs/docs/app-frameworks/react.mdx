...
Replace the contents of `src/App.tsx` with the following. The backend URL comes from the `VITE_BARGAIN_CHEF_URL` environment variable, falling back to `http://localhost:8080/bargainChefFlow`; `8080` is a common default backend port, so adjust the fallback if your backend uses a different port.
...
To point the app at a different backend, set `VITE_BARGAIN_CHEF_URL` (for example, in a `.env` file at the project root):

```bash title=".env"
VITE_BARGAIN_CHEF_URL=http://localhost:3000/api/bargainChefFlow
```
