...
**concurrently**, one goroutine per call, and waits for all of them. A tool
that fails ends the round as soon as its error arrives: the call returns while
the siblings finish on their own, and their results are discarded with the round.
...
- **Cancellation reaches the provider.** When the context is done, the in-flight
  ...
  promptly rather than at the provider's own timeout. It returns the partial
  response beside the error, marked `ai.FinishReasonAborted` and holding the
  tool rounds that completed, so a caller can pick up where the deadline hit.
- **Detached agent work outlives the request.** An agent turn started with
  `RunDetached` runs on a context the request does not own, and only an abort
  or the process exiting cancels it. See
  [Background execution](/docs/agents/background).
...
is already cancelled by the time you get there. A process running detached agent
work should abort those tasks first and wait for them to settle, so their
snapshots land as `aborted`, which resumes, rather than `expired`, which does
not.
