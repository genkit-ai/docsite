...
description: Learn how to deploy Genkit flows to any supported hosting platform with maximum flexibility and control.
supportedLanguages:
  - js
  - go
  - dart
...
```bash
npm install --save genkit @genkit-ai/google-genai @genkit-ai/express
...
1.  In the Cloud console, [Enable the Gemini Enterprise API (`aiplatform.googleapis.com`)](https://console.cloud.google.com/apis/library/aiplatform.googleapis.com?project=_)
    for your project.

2.  On the [IAM](https://console.cloud.google.com/iam-admin/iam?project=_)
    page, create a service account for accessing the Gemini Enterprise API if you
    don't already have one.

    Grant the account the `roles/aiplatform.user` IAM role.
...
4.  Configure the plugin with your Google Cloud project ID and the Gemini
    Enterprise API location you want to use. You can do so either by setting the
