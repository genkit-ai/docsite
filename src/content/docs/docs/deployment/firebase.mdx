...
description: Learn how to deploy Genkit flows as Cloud Functions for Firebase with built-in authentication and authorization.
supportedLanguages:
  - js
---
...
[Cloud Functions for Firebase](https://firebase.google.com/docs/functions) is a serverless framework that lets you run backend code in response to HTTPS requests and Firebase events without managing servers. It includes an `onCallGenkit` helper that wraps a Genkit [flow](/docs/flows/) as a [callable function](https://firebase.google.com/docs/functions/callable?gen=2nd) with built-in streaming support, declarative authorization policies, and Firebase App Check integration. Client applications can invoke these functions using `genkit/beta/client` or the [Cloud Functions client SDK](https://firebase.google.com/docs/functions/callable?gen=2nd#call_the_function), which automatically attaches Firebase Authentication and App Check tokens.
...
- No prior experience with Firebase or Cloud Functions for Firebase is required—this guide walks you through setting up a Firebase project from scratch.
