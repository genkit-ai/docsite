...
`executeFlowWorkflow` to start a workflow and `await` its result:
...
If you don't want to block on the result, use `startFlowWorkflow` instead. It
...
- `executeFlowWorkflow(flow, input, options?)` — run a flow inside a Workflow
