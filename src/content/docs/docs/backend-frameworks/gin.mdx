...
- **Retry middleware:** `genkit.Init` registers `&middleware.Middleware{}` so the built-in middleware (retry, fallback, and more) is available in the Developer UI, and the `ai.WithUse(&middleware.Retry{MaxRetries: 3})` line applies retry middleware that recovers from transient errors like rate limits and timeouts.
