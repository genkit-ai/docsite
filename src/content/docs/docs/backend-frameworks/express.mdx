...
In this guide, you'll build **Bargain Chef**, a standalone Genkit backend on Express that exposes a recipe-generating flow over HTTP. It uses two AI patterns Genkit simplifies: streaming structured output and tool calling.
...
<Aside type="note" title="Prefer a different model?">
  This guide uses Gemini, but Genkit also supports [Anthropic (Claude)](/docs/integrations/anthropic),
