...
Because the streamed response uses `reply.hijack()`, the adapter copies the headers that plugins like `@fastify/cors` set on the reply onto the raw response before piping. Without that step, a streamed response would arrive without its CORS headers, and a browser would reject it even though the preflight request succeeded.
