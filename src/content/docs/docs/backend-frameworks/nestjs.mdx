...
- **Shared TypeScript types:** `Recipe`, `PartialRecipe`, and `BargainChefInput` are inferred from the Zod schemas with `z.infer<...>` and re-exported for the NestJS controller to import, so the flow and the code that calls it stay in sync.
...
- **`use: [retry(...)]`:** Applies retry middleware that recovers from transient errors like rate limits and timeouts.
