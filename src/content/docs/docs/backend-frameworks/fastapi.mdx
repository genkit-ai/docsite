...
- **Shared Python types:** the Pydantic models (`Recipe`, `RecipeIngredient`, `CravingInput`) define the request and response shapes once, so the flow, the HTTP handler, and any client share a single source of truth.
- **The `get_ingredients_on_sale` tool:** the model decides when to call it based on the prompt, and the typed `DayType` input forces the model to pass `day_type='weekday'` or `'weekend'`. In a real app, the tool would query a pricing database, inventory system, or third-party API.
