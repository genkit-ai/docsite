...
```dart
final turn = orchestratorAgent.chat().sendTextStream('Research quicksort and write it in Dart.');

await for (final chunk in turn.stream) {
  for (final req in chunk.toolRequests) {
    if (req.name.startsWith('delegate_to_')) {
      print('Delegating to sub-agent: ${req.name}');
    }
  }
