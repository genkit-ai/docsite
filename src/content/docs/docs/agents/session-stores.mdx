...
Import `FileSessionStore` from the IO-safe entry point: `package:genkit/experimental_io.dart`.
...
- **`getSnapshot`** fetches a snapshot by exact `snapshotId` or resolves the latest snapshot in the sequence for `sessionId`. The optional `context` carries ambient request/action context (such as the authenticated user) for multi-tenant routing.
...
- **`SnapshotMetadataReader`** (optional): Implement this interface (`getSnapshotMetadata` and `getLatestSnapshotMetadata`) if your database supports projection queries, allowing `metadataOnly: true` status checks to skip loading the full conversation `state` payload.
