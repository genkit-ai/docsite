...
class MyDatabaseSessionStore(SessionStore):
    ...
    @property
    def lock(self):
        # Return a process-local asyncio.Lock for this store instance.
        ...
```

- **`get_snapshot`** fetches a snapshot by exact `snapshot_id` or resolves the latest snapshot in the sequence for `session_id`.
- **`save_snapshot`** must be atomic. Run the read, mutator execution, and write inside a single database transaction. The mutator must be side-effect free — stores may call it more than once under contention.
