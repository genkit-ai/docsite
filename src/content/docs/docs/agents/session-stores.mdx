...
Use the in-memory store for tests, demos, local examples, and single-process experiments. Use the file store for local development, prototypes, CLIs, and single-host apps that need snapshots to survive process restarts. Use a custom store for production web apps that need a database, cloud storage, centralized authorization, retention policies, or tenant-aware persistence.
...
The JavaScript in-memory store accepts one option:
...
Snapshots are written under the directory passed to the constructor. By default, JavaScript stores them under a `global` subdirectory. When you provide `snapshotPathPrefix`, the prefix determines the subdirectory for every read and write.
...
Implement a custom `SessionStore` when snapshots should live in your production data layer. This is the right approach for Cloud SQL, Firestore, Spanner, Postgres, Redis-backed systems with durability, or application-specific storage that already handles user and tenant authorization.

A custom JavaScript store implements three capabilities:
...
For JavaScript stores, "latest" means the latest leaf snapshot in the session history. A leaf is a snapshot that no other snapshot references as its parent. If branching exists, the built-in stores either select the most recently created leaf or throw when `rejectBranchingSessions` is enabled.
...
Index by snapshot ID, session ID, parent ID, and creation time. `sessionId` lookup should be efficient because it is the common path for continuing a conversation. Parent relationships matter for JavaScript leaf selection.
...
The Go in-memory store does not have configuration options. If you need retention or tenant-scoped local files, use the file store.
...
A custom Go store implements snapshot reading and writing:
...
`GetLatestSnapshot` loads the most recently created snapshot for a session, whatever its status. Use `CreatedAt` for recency, not `UpdatedAt`, because heartbeat writes update liveness without changing the conversation state. If two snapshots have the same `CreatedAt`, break ties deterministically. Parent IDs are lineage metadata for this lookup, not the way Go resolves the latest session snapshot.
...
Scope every read and write by authenticated user, organization, or tenant. Do not rely on snapshot IDs alone as authorization. In Go, derive tenancy from `context.Context` and apply it consistently in every store method.
