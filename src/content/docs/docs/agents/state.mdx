...
Tools and custom agents can update custom state using the functional `session.updateCustom()` API. Genkit automatically diffs the result and streams RFC 6902 JSON Patches to the client mid-stream.
...
session.updateCustom((custom) {
  ...
  state['tasks'].add({'id': 1, 'title': 'Buy milk', 'done': false});
  return state;
});
```

Keep custom state serializable, and provide a `stateSchema` when defining your agent to validate state structure at load time.
...
await chat.sendText('What is the weather in Tokyo?');
```
...
await branch.sendText('Assume the user changed their mind.');
```
...
final res = await chat.sendText('Add a task.');
// Store res.state on the client (e.g., local storage or database)
saveStateOnClient(res.state);
```
...
```dart
final turn = taskAgent.chat().sendTextStream('Add buy milk to my list.');
