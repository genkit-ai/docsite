...
The `barista.go` file of [`go/samples/basic-agents`](https://github.com/genkit-ai/genkit/tree/main/go/samples/basic-agents) runs this pair end to end. Note the `omitempty` on the state's slice field: a nil Go slice marshals to `null`, which does not satisfy the array schema inferred from the type.
...
On every successful turn, the store saves a `completed` snapshot capturing the conversation's exact state. If a turn fails or is aborted, the store saves a `failed` or `aborted` snapshot carrying the error details (on failure) and the last-good state from before that turn so it can be rerun without corruption.

For store choices, see [Session stores](/docs/agents/session-stores).

## Snapshots

Read a snapshot directly by ID or fetch the latest snapshot in a session using the agent's snapshot methods. Pass `metadataOnly: true` to read status and metadata without loading the conversation state:

```dart
// Read a specific snapshot point:
final snapshot = await weatherAgent.getSnapshot(snapshotId: 'snapshot-123');

// Read metadata only for the latest point in a conversation:
final latest = await weatherAgent.getSnapshot(
...
| ----------- | ------------------------------------------------------------------------------------------- |
| `pending`   | A detached background invocation is still running.                                          |
| `aborting`  | An abort has been requested; the background worker is unwinding and saving its progress.    |
| `completed` | The snapshot captures a settled, resumable state.                                           |
| `failed`    | The turn failed. Error details and the last-good history before the turn are stored.        |
| `aborted`   | The turn was cancelled or hit `maxTurns`. The last-good history before the turn is stored. |
| `expired`   | A pending snapshot heartbeat went stale, so the background worker is presumed dead.         |

`completed`, `failed`, and `aborted` snapshots are valid resume points (`expired` is terminal and not resumable). Other statuses are useful for UI progress indicators and background tracking.
