...
def mutate(custom: dict | None) -> dict:
    state = custom or {}
    next_id = state.get('next_id') or 1
    tasks = list(state.get('tasks') or [])
    tasks.append(TaskItem(id=next_id, title='Buy milk').model_dump())
...
await chat.send('What is the weather in Tokyo?').response
```
...
await branch.send('Assume the user changed their mind.').response
```
...
res = await chat.send('My name is Ada. Remember it.').response
...
await resumed.send('What is my name?').response
```
...
```python
turn = task_agent.chat().send('Add buy milk to my list.')
