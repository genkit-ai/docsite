...
The client is independent of the backend language. Point it at the primary turn URL your server exposes, such as `/api/weatherAgent` for the Express routes above or `/agents/{name}` for the Go routes, and it speaks the same wire protocol either way. The snapshot and abort companion URLs follow your server's route layout.
