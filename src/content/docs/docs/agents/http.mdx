...
@Component({
  ...
  template: `
    ...
    <form (submit)="$event.preventDefault(); send()">
      <input [ngModel]="input()" (ngModelChange)="input.set($event)" name="input" />
...
Agent custom state does not appear in `messages`. When the agent updates its session state, the transport emits it as a transient `data-custom` part delivered through the binding's `onData` callback, not added to a message, so a UI that only renders `messages` never sees it. Read `part.data` for the full, current custom state. Both `useChat(options)` and `new Chat(options)` take `onData` in the same options object as `id` and `transport`:
