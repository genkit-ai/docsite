...
final agent = remoteAgent(
  url: 'http://localhost:8080/api/weatherAgent',
  getSnapshotUrl: 'http://localhost:8080/api/weatherAgent/state',
...
- **`url`** is required and sends normal and streaming turns.
- **`getSnapshotUrl`** specifies the endpoint to load saved snapshots for server-managed agents (often mapped to `/state`).
- **`abortUrl`** specifies the endpoint to cancel detached background turns.
- **`headers`** can be a function or map that returns custom headers, resolving auth tokens dynamically per-request.

## Stream a turn

`sendTextStream()` returns a turn that exposes a chunk stream and a final response.

```dart
final chat = agent.chat();
final turn = chat.sendTextStream('Write a long report.');
