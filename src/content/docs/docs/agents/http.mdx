...
With the agent behind an AI SDK UI binding, you drive it from the SDK's chat primitives instead of wiring up `remoteAgent()` yourself, and you can build the interface from Vercel's [AI Elements](https://ai-sdk.dev/elements) chat components, which are built on those same bindings.
