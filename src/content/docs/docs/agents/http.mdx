...
`@genkit-ai/vercel-ai` adapts an agent to the [Vercel AI SDK](https://ai-sdk.dev). `GenkitChatTransport` implements the SDK's framework-agnostic `ChatTransport`, so it works with any of the SDK's UI bindings, including React, Vue, Svelte, and Angular. You drive the agent with the SDK's chat primitives and render messages with its UI components instead of wiring up `remoteAgent()` yourself. The transport speaks the same wire protocol over the agent route, so it works against a JavaScript or Go backend.
