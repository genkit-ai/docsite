...
Importing both `package:genkit/experimental_client.dart` and `package:genui/genui.dart` causes collisions on `basicCatalogId` and `DataPart`. Hide them when importing `genui`: `import 'package:genui/genui.dart' hide basicCatalogId, DataPart;`.
:::
