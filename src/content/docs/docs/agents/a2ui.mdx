...
Flutter applications render A2UI surfaces using [`genui`](https://pub.dev/packages/genui). Client components use `package:genkit/experimental_client.dart`, `package:genkit_a2ui/client.dart`, and `package:a2ui_core/a2ui_core.dart`.
...
Importing both `package:genkit_a2ui/client.dart` and `package:genui/genui.dart` causes a collision on `basicCatalogId` (and on `DataPart` if you also import `package:genkit/client.dart`). Hide them when importing `genui`: `import 'package:genui/genui.dart' hide basicCatalogId, DataPart;`.
:::
