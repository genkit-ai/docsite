...
```dart
final chat = agent.chat(sessionId: 'report-123');
final task = await chat.detach(
  agentInputFromText('Write the quarterly market report.'),
);
...
```dart
await for (final snapshot in task.poll(intervalMs: 1000)) {
  renderStatus(snapshot.status);
...
```dart
final finalSnapshot = await task.wait(intervalMs: 1000);
...
if (snapshot?.status == 'completed') {
  final chat = await agent.loadChat(snapshotId: snapshotId);
  await chat.send(agentInputFromText('Summarize the report in three bullets.'));
}
```
