...
void main() {
  ...
  router.post('/api/reportAgent/state', shelfHandler(reportAgent.getSnapshotDataAction));
...
Submit a background task from the client using `detachText()` on the `AgentChat`:

```dart
final chat = reportAgent.chat(sessionId: 'report-123');
final task = await chat.detachText('Write the quarterly market report.');
...
```dart
await for (final snapshot in task.poll(intervalMs: 1500)) {
  print('Current Status: ${snapshot.status?.value}');

  if (snapshot.status?.value == 'completed') {
    final report = snapshot.state?.messages?.last.content.first.text;
    print(report);
  }
}
```
...
```dart
final finalSnapshot = await task.wait(intervalMs: 1500);
...
if (snapshot?.status?.value == 'completed') {
  ...
  final res = await chat.sendText('Summarize this report.');
  print(res.text);
}
```
