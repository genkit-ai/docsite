...
Pass `metadataOnly: true` to `task.poll(...)` or `reportAgent.getSnapshot(...)` when you only need status, timestamps, or error info without transferring the full conversation state:

```dart
final statusSnap = await reportAgent.getSnapshot(
...
Non-terminal statuses are `pending` and `aborting`. Terminal statuses are `completed`, `failed`, `aborted`, and `expired`.
...
if (snapshot?.status?.value == 'completed') {
  ...
  print(res.text);
} else if (snapshot?.status?.value == 'failed' ||
    snapshot?.status?.value == 'aborted') {
  // Failed and aborted snapshots preserve last-good progress and can be rerun:
  final rerun = reportAgent.chat(snapshotId: snapshotId);
  await rerun.send();
}
```

`completed`, `failed`, and `aborted` snapshots are valid resume points (`expired` is terminal and not resumable; resume from its `parentId` instead).
...
Aborting transitions a running `pending` snapshot to `aborting` while the background worker unwinds and saves its completed turns, then commits an `aborted` snapshot holding the last-good state.
