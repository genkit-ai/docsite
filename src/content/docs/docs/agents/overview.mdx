...
async def main() -> None:
    chat = assistant.chat()
    res = await chat.send('Hello. What can you do?').response
    print(res.text)
...
1. Define the agent with a model, system instructions, and tools.
2. Call it locally with `chat().send()`, then consume `turn.stream` and/or `turn.response`.
3. Add a session store (`InMemorySessionStore` or `FileSessionStore`) when the server should own history.
...
- [Run and stream](/docs/agents/run) covers `chat()`, `load_chat()`, `send`, streaming, and `AgentError`.
