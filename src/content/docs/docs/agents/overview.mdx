...
void main() async {
  ...
  final res = await chat.sendText('Hello. What can you do?');
...
2. Call it locally with `chat.sendText()` or `chat.sendTextStream()`.
...
- [Run and stream](/docs/agents/run) covers `chat()`, `loadChat()`, `sendText`, `sendTextStream`, and `AgentError`.
...
- [Background execution](/docs/agents/background) covers detached background work (`detachText`), polling, and aborting.
