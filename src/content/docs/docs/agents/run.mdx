...
await chat.send('What did we discuss last time?').response
```
...
await chat.send('Continue from there.').response
```

Use `get_snapshot()` when you only need to inspect a snapshot, such as for an audit view. Use `load_chat()` when you want to continue the conversation from that saved state.
...
`chat.send()` returns an `AgentTurn`. Iterate `turn.stream` for live chunks, or await `turn.response` for the final output. Both paths apply custom-state patches so `chat.state` stays current.
...
try:
    await chat.send('Use a broken tool.').response
