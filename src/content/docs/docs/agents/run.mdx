...
`AgentAPI` is the callable agent. It creates chats, loads snapshots, reads snapshots, and aborts detached work.

`AgentChat` is a stateful conversation. It sends normal turns, streams turns, resumes interrupts, detaches work, and tracks the next-turn state for you.

`AgentTurn` represents one in-flight streaming turn. It gives you a stream, a final response promise, and an abort helper.

`AgentResponse` is the completed turn. It includes text, tool requests, interrupts, finish reason, snapshot ID, session ID, custom state, artifacts, raw output, and `assertValid()`.
...
Use `aix.WithSessionID[State](id)` to resume the latest server-managed snapshot for a conversation.

Use `aix.WithSnapshotID[State](id)` to resume or branch from a specific server-managed snapshot.

Use `aix.WithState[State](state)` to continue a client-managed conversation by sending the full state.
...
Reach for `Connect` when the caller needs direct control over both sides of the conversation on one live connection. It is useful for Go CLIs, local services, workers, and lower-level integrations that need to stream output, observe custom state patches, handle interrupts, send a resume payload, or send another message after a `TurnEnd` without reconnecting.

For most single-turn server code, use `RunText` or `Run`. For browser apps and HTTP clients, use the JavaScript `chat.send()` and `chat.sendStream()` helpers through `remoteAgent()` rather than managing a bidirectional stream directly.
