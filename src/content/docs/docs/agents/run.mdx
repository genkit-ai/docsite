...
await chat.sendText('What did we discuss last time?');
```
...
await chat.sendText('Continue from there.');
```
...
The non-streaming `sendText()` path drains the stream internally so custom state patches are still applied. This keeps `sendText()` and `sendTextStream()` consistent for server-managed agents.

## Pass per-turn context

Every turn method (`sendText`, `sendTextStream`, `detachText`, and their message-based equivalents) accepts an optional `context` map. Use it to pass ambient request data, such as auth, that tools and custom agents can read without exposing it to the model.
...
```dart
try {
  await chat.sendText('Use a broken tool.');
