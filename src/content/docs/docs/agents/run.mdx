...
The JavaScript and Dart clients expose the same high-level interface, whether the agent runs locally (JavaScript `ai.defineAgent()`) or behind HTTP through `remoteAgent()`. In Go, the in-process API uses `RunText` and `Connect` instead, covered below.
