...
Most Genkit agents should use the standard prompt-backed loop. Custom orchestration is for cases where the application must control turn processing directly while still using the Agents API for sessions, snapshots, streaming, HTTP transport, and background execution.

If you need complete ownership of the backend contract, a Genkit flow with direct `generate()` calls may be a better fit. Use a custom agent when you need custom orchestration but still want the agent contract for persistent conversations, iterative generation, frontend streaming, resumable state, interrupts, and background work.
...
| Method                    | Purpose                                        |
| ------------------------- | ---------------------------------------------- |
| `getState()`              | Full session state.                            |
| `getMessages()`           | Conversation messages.                         |
| `addMessages(messages)`   | Append messages to history.                    |
| `setMessages(messages)`   | Replace message history.                       |
| `getCustom()`             | Current custom state.                          |
| `updateCustom(fn)`        | Mutate custom state and stream a custom patch. |
| `getArtifacts()`          | Current artifacts.                             |
| `addArtifacts(artifacts)` | Add or replace artifacts.                      |
...
| Value                            | Purpose                                                          |
| -------------------------------- | ---------------------------------------------------------------- |
| `ctx`                            | Invocation context, canceled on client disconnect or abort.      |
...
| Field              | Purpose                                                                          |
| ------------------ | -------------------------------------------------------------------------------- |
| `SnapshotID`       | The snapshot ID reserved before this turn runs. Empty for client-managed agents. |
| `ParentSnapshotID` | The snapshot this turn continues from.                                           |
| `TurnIndex`        | Zero-based turn number within the invocation.                                    |
