...
| Category                  | How it surfaces                                                              | Recovery                                                                            |
| ------------------------- | ---------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| Init misuse               | Thrown error before a turn starts                                            | Fix the caller. For example, do not send `state` to a store-backed agent.           |
| Failed turn               | `AgentError` with `response`, `status`, `details`, `state`, and `snapshotId` | Resume from the last-good state or snapshot.                                        |
| Foreground abort          | Response with `finishReason: 'aborted'`                                      | Let the user retry.                                                                 |
| Background failure        | Snapshot status `failed`, `aborted`, or `expired`                            | Show status, inspect snapshot error, retry from a completed snapshot when possible. |
| Domain-level tool problem | Structured tool output                                                       | Let the model explain or ask the user for corrected input.                          |
...
| Category            | How it surfaces                                      | Recovery                                                                                 |
| ------------------- | ---------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| Rejected init       | Non-nil Go error from `Connect`, `Run`, or `RunText` | Fix the caller or selected resume source.                                                |
| Failed turn         | `AgentOutput` with `FinishReasonFailed` and `Error`  | Resume from `State` for client-managed agents or `SnapshotID` for server-managed agents. |
| Tool domain problem | Structured tool output                               | Let the coordinator or model explain and ask for corrected input.                        |
| Background failure  | Snapshot status `failed`, `aborted`, or `expired`    | Inspect snapshot error or retry from a completed snapshot.                               |
...
```go
func lookupOrder(ctx context.Context, input LookupOrderInput) (LookupOrderOutput, error) {
	order, err := db.LookupOrder(ctx, input.OrderID)
	if err != nil {
		return LookupOrderOutput{}, err
	}
	if order == nil {
