...
try:
    res = await chat.send('Look up order 123.').response
    print(res.text)
    ...
    await recovery_chat.send('Try again with order 456.').response
```
...
@ai.tool(name='lookupOrder', description='Looks up an order by ID.')
