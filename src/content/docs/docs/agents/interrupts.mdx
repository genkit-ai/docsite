...
banking_agent = ai.define_agent(
    ...
    system='If the user wants to transfer money, ALWAYS use userApproval before transferMoney.',
...
@ai.tool(name='run_shell', description='Run a shell command after a safety check.')
...
```python
res = await chat.send('Transfer $500 to savings.').response
...
Use `respond()` when the client already has the final tool output. Pass the part to `chat.resume(respond=...)`. The tool does not run again on the server.
...
```python
res = await chat.send('Deploy code to production.').response
