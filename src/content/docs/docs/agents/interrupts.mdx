...
Use `respond()` when the client has final tool outputs ready. This builder returns a `ToolResponsePart` which you transmit using `chat.resume(interruptRespond: ...)`. This resolves the tool call directly without re-executing the tool function on the server.

```dart
final res = await chat.sendText('Transfer $500 to savings.');
final approval = res.interrupts.firstWhereOrNull((i) => i.name == 'userApproval');

if (approval != null) {
  final continued = await chat.resume(interruptRespond: [
...
```dart
final res = await chat.sendText('Deploy code to production.');
final deployInterrupt = res.interrupts.firstWhereOrNull((i) => i.name == 'deployApproval');

if (deployInterrupt != null) {
  final continued = await chat.resume(interruptRestart: [
    deployInterrupt.restart(),
  ]);
  print(continued.text);
}
```
