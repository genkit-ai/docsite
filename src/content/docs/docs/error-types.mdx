...
| `ErrToolFailed`            | `status.ErrInternal`       | A tool returned an error, or its output did not match its declared schema. The loop's partial response rides alongside it.           |
| `ErrMaxTurnsExceeded`      | `status.ErrAborted`        | The tool loop hit its turn limit before the model produced a final response. Raise the limit with `ai.WithMaxTurns`, or continue from the partial response's `History()`. |
...
### Generation errors carry the partial response

Once a generate request has resolved, an error arrives beside the partial
`*ai.ModelResponse` rather than a nil one. The response reports
`ai.FinishReasonFailed` when something broke and `ai.FinishReasonAborted` when
the caller stopped the loop, carries the same failure classified in
`resp.Error`, and its `History()` ends at the last completed tool round, so a
caller can send it again without repeating work. Errors raised before the
request was sent, such as `ErrModelNotFound`, still come with a nil response.
See [Failed and stopped generations](/docs/models#failed-and-stopped-generations).
...
```go
recipe, resp, err := genkit.GenerateData[Recipe](ctx, g, ai.WithPrompt("Suggest a recipe."))
if errors.Is(err, status.ErrInvalidOutput) {
	// The model answered, but not in the shape we asked for. resp.Text()
	// holds what it wrote, so the retry can show it what to fix.
}
```
...
handled for you (100 inputs per call on the Gemini API, up to 250 on the Gemini Enterprise API), so an
...
`ai.generate()` and `ai.generateStream()` do not throw for model errors or a
tool that throws. Instead they resolve to a response with
`finishReason: FinishReason.failed`, carrying a structured `error` and the
last-good conversation history in `messages`.
...
if (res.finishReason == FinishReason.failed) {
  // res.error is a RuntimeError. A thrown GenkitException keeps its status;
  // anything else maps to INTERNAL.
  print(res.error?.status);
  // res.cause holds the original thrown object (in-process only).
  print(res.cause);

  // res.messages holds the last-good state: the request messages plus any
  // completed tool turns, stopping before the turn that failed. Feed it back
  // into a fresh generate() to continue from that point.
...
Cancellation and hitting `maxTurns` resolve to `FinishReason.aborted` rather
than throwing. These responses also carry the last-good state in `res.messages`,
so you can continue execution from the abort point the same way. See
[Generate content with AI models](/docs/models) for the full error, cancellation,
and resume flow.
