...
To use a prompt, first load it using the `prompt('file_name')` method:
    ...
    ```python
    stream, response = hello_prompt.stream()
...
Use the `define_prompt()` function. You can specify a Handlebars template string
    ...
    ```python
    stream, response = geography_prompt.stream(GeoQuery(country_count=15))
    ...
    You can also define prompts with multi-turn messages:
