...
Register each middleware by name with `ai.DefineMiddleware` so the `use`
reference resolves:
...
ai.DefineMiddleware(g, "retry", &middleware.Retry{MaxRetries: 3})
ai.DefineMiddleware(g, "skills", &middleware.Skills{SkillPaths: []string{"./skills"}})
```
