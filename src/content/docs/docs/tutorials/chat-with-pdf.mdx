...
import { genkit } from 'genkit/beta'; // chat and agents are beta features
...
- The second line imports `genkit` from `'genkit/beta'` because conversational
  sessions (`defineAgent` and `chat`) are currently part of the beta API
  surface.
- The next two lines import `PDFParse` from the `pdf-parse` library for parsing
  PDF files and the `fs` module for file system operations.
...
```typescript
// Step 4: start chat
const pdfAgent = ai.defineAgent({
  name: 'pdfChat',
  system: prompt,
});
const chat = pdfAgent.chat();
const readline = createInterface(process.stdin, process.stdout);
console.log("You're chatting with Gemini. Ctrl-C to quit.\n");
```

The `defineAgent` and `chat` calls initialize a stateful chat session using your
system prompt (which includes the full text of the PDF file). The rest of the
code instantiates a text input and displays a message to the user.
