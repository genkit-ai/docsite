...
- **Integration with developer UI**: Debug flows independently of your
  ...
  - **Simplified deployment**: Deploy flows directly as web API endpoints to any platform.
...
You can define schemas in much the same way as you define the output schema of a
...
You can define schemas using Pydantic.
...
contain `generate()` calls). Here's a variation of the example that uses the structured output to format a simple string, which the flow returns.
...
. Flow's `stream` method returns the stream async iterable,
