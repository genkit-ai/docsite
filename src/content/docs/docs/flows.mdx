...
<Lang lang="js go python dart" terms={{
  "schemaLibraryReference": {
    "js": "[Zod](https://zod.dev/)",
    "go": "Go structs",
    "python": "[Pydantic Models](https://docs.pydantic.dev/latest/concepts/models/)",
    "dart": "[Schemantic](https://pub.dev/packages/schemantic)"
  }
}}>
- **Type-safe inputs and outputs**: Define schemas using [[schemaLibraryReference]] for static and runtime validation.
...
export const menuSuggestionFlow = ai.defineFlow(
  ...
  async ({ theme }) => {
    const { text } = await ai.generate({
      model: googleAI.model("[[@global.models.geminiFlash]]"),
  ...
  final menuSuggestionFlow = ai.defineFlow(
    ...
    fn: (input, _) async {
      final response = await ai.generate(
        model: googleAI.gemini('[[@global.models.geminiFlash]]'),
...
<Lang lang="go python" terms={{
  ...
  "developerExperienceSubject": {
    "go": "Struct-based schemas",
    "python": "Model-based schemas"
  },
  "futureProofSubject": {
    "go": "Struct-based schemas",
    "python": "Model-based schemas"
...
You can define schemas using [[schemaType]]. While you can use primitive types directly as input and output parameters, it's considered best practice to use [[schemaStyle]] schemas for these reasons:

- **Better developer experience**: [[developerExperienceSubject]] provide a better experience in the Developer UI by giving you labeled input fields.
- **Future-proof API design**: [[futureProofSubject]] allow for easy extensibility in the future. You can add new fields to your input or output schemas without breaking existing clients, which is a core principle of robust API design.
</Lang>
<Lang lang="dart">
...
<Lang lang="js go dart" terms={{
  "schemaStyle": {
    "js": "object-based",
    "go": "struct-based",
    "dart": "Schematic-based"
  }
}}>
All examples in this documentation use [[schemaStyle]] schemas to follow these best practices.
</Lang>
...
<Lang lang="js go" terms={{
  "durableStreamingSuffix": {
    "js": "",
    "go": " Note that durable streaming for Go is currently experimental."
  }
}}>
:::tip[Durable Streaming]
For flows that run for a long time or where network reliability is a concern, you can use [durable streaming](/docs/durable-streaming). This allows the client to reconnect to a stream and replay the content.[[durableStreamingSuffix]]
:::
</Lang>
...
<Lang lang="js go python dart" terms={{
  "flowName": {
    "js": "menuSuggestionFlow",
    "go": "menuSuggestionFlow",
    "python": "menu_suggestion_flow",
    "dart": "menuSuggestionFlow"
  }
}}>
```bash
genkit flow:run [[flowName]] '{"theme": "French"}'
```
...
```bash
genkit flow:run [[flowName]] '{"theme": "French"}' -s
```
