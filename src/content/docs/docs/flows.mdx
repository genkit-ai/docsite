...
flow. Flows are written just like functions, but they add additional capabilities intended to ease the development of AI features:

<LanguageContent lang="js">

- **Type safety**: Input and output schemas defined using Zod, which provides both static and runtime type checking
- **Streaming**: Flows support streaming of data, such as partial LLM responses, or any custom serializable objects
...
- **Streaming**: Flows support streaming of data, such as partial LLM responses, or any custom serializable objects
- **Integration with developer UI**: Debug flows independently of your application code using the developer UI. In the developer UI, you can run flows and view traces for each step of the flow.
- **Simplified deployment**: Deploy flows directly as web API endpoints to any platform that can host a web app.
...
standard code, and code inside a flow doesn't need to be flow-aware.
...
@ai.flow()
async def menu_suggestion_flow(input: MenuSuggestionInput, ctx) -> MenuOutput:
...
To deploy flows using Flask, you can use the `genkit_flask_handler` decorator to expose your flows as HTTP endpoints:
...
@ai.flow()
async def menu_suggestion_flow(input: MenuSuggestionInput, ctx) -> MenuSuggestionOutput:
    response = await ai.generate(
        prompt=f'Invent a menu item for a {input.theme} themed restaurant.',
...
The `genkit_flask_handler` decorator automatically handles:
...
```bash
gunicorn --bind 0.0.0.0:3400 --workers 4 main:app
```
