...
You can also expose AI models as REST API endpoints using `expressHandler`. This allows you to serve models that can be consumed by other Genkit applications using [`defineRemoteModel`](/docs/models#consuming-remote-models) (or directly via HTTP clients).
...
### Using `withFlowOptions`

For more advanced authentication scenarios, you can use `withFlowOptions` to wrap your flows with a `ContextProvider`. This allows you to inject custom context, such as authentication details, into your flows.
...
const customContextProvider: ContextProvider<CustomAuthContext> = async (
  req: RequestData,
) => {
...
export const protectedFlow = ai.defineFlow(
  ...
  async ({ input }, { context }) => {
    ...
    return {
      output: `Hello, ${context.auth.user}! Your role is ${context.auth.role}. You said: ${input}`,
    };
  },
);
...
startFlowServer({
  flows: [withFlowOptions(protectedFlow, { contextProvider: customContextProvider })],
});
```
...
The `apiKey` context provider is a built-in `ContextProvider` that allows you to perform API key-based access checks. It can be used with `withFlowOptions` to secure your flows.
...
startFlowServer({
  flows: [withFlowOptions(securedFlow, { contextProvider: apiKey(process.env.MY_API_KEY) })],
});
```
