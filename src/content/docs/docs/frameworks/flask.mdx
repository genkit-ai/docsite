...
@genkit_flask_handler(ai)
@ai.flow()
async def structured_joke_flow(request: JokeRequest, ctx: ActionRunContext | None = None) -> JokeResponse:
...
@genkit_flask_handler(ai)
@ai.flow()
async def story_flow(prompt: str, ctx: ActionRunContext | None = None):
    ...
    response = await ai.generate(
        ...
        on_chunk=ctx.send_chunk if ctx else None,  # This enables streaming
    )
...
```python
from genkit import GenkitError, ActionRunContext
from genkit.plugin_api import RequestData
import jwt

def auth_context_provider(req: RequestData):
    """Custom authentication context provider."""
    # req is a RequestData object wrapping the Flask request.
    # Its headers dictionary uses lowercased keys.
...
@ai.flow()
async def protected_flow(message: str, ctx: ActionRunContext):
...
def require_role(required_role: str):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            # Bind arguments to extract ctx robustly
            sig = inspect.signature(func)
            bound_args = sig.bind(*args, **kwargs)

            ctx = None
            for arg_val in bound_args.arguments.values():
                if isinstance(arg_val, ActionRunContext):
                    ctx = arg_val
                    break
...
@ai.flow()
@require_role('admin')
async def admin_flow(action: str, ctx: ActionRunContext):
    """Admin-only endpoint."""
    user = ctx.context['user']
...
@genkit_flask_handler(ai)
@ai.flow()
async def safe_flow(input_text: str, ctx: ActionRunContext | None = None):
...
def register_routes(app, ai):
    ...
    @genkit_flask_handler(ai)
    @ai.flow()
    async def generate_flow(prompt: str, ctx: ActionRunContext | None = None):
        return await ai.generate(
            prompt=prompt,
            on_chunk=ctx.send_chunk if ctx else None,
        )
...
@genkit_flask_handler(ai)
@ai.flow()
async def chat_flow(message: str, ctx: ActionRunContext | None = None):
    return await ai.generate(
...
@genkit_flask_handler(ai)
@ai.flow()
async def limited_flow(input_data: str, ctx: ActionRunContext | None = None):
    return await ai.generate(
