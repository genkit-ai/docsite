...
FastAPI supports streaming responses using Server-Sent Events (SSE):
...
@ai.flow()
async def streaming_menu_flow(theme: str, ctx: ActionRunContext | None = None) -> str:
    """Generate a menu item with streaming."""
    response = await ai.generate(
        prompt=f'Invent a menu item for a {theme} themed restaurant.',
...
@app.post("/streamMenuSuggestion")
async def stream_menu(request: MenuRequest):
    """Stream a menu suggestion."""
    async def event_generator():
        ...
        ctx = ActionRunContext(send_chunk=collect_chunk)
        result = await streaming_menu_flow(request.theme, ctx)
        yield f"data: {json.dumps({'complete': True, 'text': result})}\n\n"
...
You can call your FastAPI endpoints using the Genkit client library or any HTTP client:

### Using the Genkit client

```python
from genkit.beta.client import runFlow, streamFlow

# Non-streaming call
result = await runFlow(
    url='http://localhost:8000/menuSuggestion',
    input={'theme': 'pirate'}
)
print(result['menuItem'])

# Streaming call
stream = streamFlow(
    url='http://localhost:8000/streamMenuSuggestion',
    input={'theme': 'pirate'}
)
