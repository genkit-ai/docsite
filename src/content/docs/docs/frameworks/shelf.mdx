...
<Lang lang="js">
  :::note[Unapplicable for JavaScript] Shelf is a Dart web server middleware
  ecosystem. For JavaScript applications, consider
  [Express.js](/docs/frameworks/express) or [Next.js](/docs/frameworks/nextjs).
  :::
</Lang>

<Lang lang="go">
  :::note[Unapplicable for Go] Shelf is a Dart web server middleware ecosystem
  and is not applicable to Go. :::
</Lang>

<Lang lang="python">
  :::note[Unapplicable for Python] Shelf is a Dart web server middleware
  ecosystem. For Python applications, consider [Flask](/docs/frameworks/flask).
  :::
</Lang>
...
You can also serve AI models (and other Genkit actions) using `startFlowServer` or `shelfHandler`. This allows you to expose models as web endpoints that can be consumed by other Genkit applications using [`defineRemoteModel`](/docs/models#consuming-remote-models).
```dart
import 'dart:io';
import 'package:genkit/genkit.dart';
