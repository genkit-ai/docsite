---
title: Vector Search with BigQuery
description: Learn how to use Vector Search in Gemini Enterprise with BigQuery using Genkit.
supportedLanguages:
  - js
  - go
---
...
Vector Search in Gemini Enterprise allows you to index and retrieve documents. The documents are stored in BigQuery and the corresponding document IDs are indexed using Vector Search in Gemini Enterprise. These are suitable for production use cases.
...
1. Create a Vector Search index in Gemini Enterprise. Details on creating an index can be found at [Create your Vector Search Index](https://cloud.google.com/vertex-ai/docs/vector-search/create-manage-index#create-index)
...
To use Vector Search with BigQuery, initialize it and define a retriever with an embedder. You can also use a custom indexer and retriever for indexing and retrieving documents from the BigQuery dataset:
