...
generated_image = response.media
```
...
llm_response = await ai.generate(
    messages=[
        {
            'role': 'user',
            'content': [{'text': text_content}],  # Include the large text as context
        },
        {
            'role': 'model',
            'content': [
...
from genkit.ai import Genkit
from genkit.plugins.google_genai import VertexAI
from genkit.plugins.evaluators import EvaluatorsPlugin
...
# Define an evaluator
@ai.evaluator(name='my_evaluator')
async def my_evaluator(input: str, output: str, context: list[str]) -> dict:
    # Custom evaluation logic
    return {'score': 0.85, 'reason': 'Good response'}
...
response = await ai.generate(
    prompt='What are the company benefits?',
