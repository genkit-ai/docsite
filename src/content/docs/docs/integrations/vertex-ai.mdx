---
title: Gemini Enterprise plugin
description: Learn how to use Gemini Enterprise with Genkit, including enterprise features like grounding, Vector Search, Model Garden, and evaluation metrics.
supportedLanguages:
  - js
  - go
  - dart
...
:::note
While this product is **Gemini Enterprise**, the plugin package (`genkit_vertexai`) and initializer (`vertexAI`) retain `vertex` naming from the prior brand for backward compatibility.
:::
...
The plugin requires you to specify your Google Cloud project ID, the [region](https://cloud.google.com/vertex-ai/generative-ai/docs/learn/locations) to which you want to make Gemini Enterprise API requests, and your Google Cloud project credentials.
...
- By default, `vertexAI` gets the Gemini Enterprise API location from the `GCLOUD_LOCATION` environment variable.
...
2. In addition, make sure the account is granted the `roles/aiplatform.user` IAM role. See the Gemini Enterprise [access control](https://cloud.google.com/vertex-ai/generative-ai/docs/access-control) docs.
...
:::note
While this product is **Gemini Enterprise**, the Genkit plugin packages (`@genkit-ai/vertexai`, `genkit-vertexai`), initializers (`vertexAI` / `VertexAI`), and model prefixes (`vertexai/`) retain `vertex` naming from the prior brand for backward compatibility.
:::

:::tip[Getting Started]
For simple API key access to Google's AI models, start with the [Google AI plugin](/docs/integrations/google-genai). This page covers enterprise features available via the Gemini Enterprise API.
:::
...
The following Gemini models are registered for use with the Gemini Enterprise plugin:
...
You can specify the `payGo` option to use Flex or Priority routing in Gemini Enterprise.
...
### Grounding (Gemini Enterprise Search & Google Search)

Enable Google Search or Gemini Enterprise data stores to provide answers grounded in verifiable sources.
...
// Gemini Enterprise Search Grounding
...
Access third-party models through Model Garden in Gemini Enterprise:
...
Use the Gemini Enterprise Rapid Evaluation API for model evaluation:
...
Use Vector Search in Gemini Enterprise for enterprise-grade vector operations:
...
:::caution[Pricing]
Vector Search has both ingestion and hosting costs. See [Gemini Enterprise pricing](https://cloud.google.com/vertex-ai/pricing#vectorsearch) for details.
:::

## Next Steps

- Learn about [generating content](/docs/models) to understand how to use these models effectively
- Explore [evaluation](/docs/evaluation) to leverage Gemini Enterprise evaluation metrics
...
Gemini Enterprise and the Gemini Developer API are served by the same `googlegenai` package; the
plugin value you pass to `genkit.Init` (`&googlegenai.VertexAI{}` vs. `&googlegenai.GoogleAI{}`) decides which backend you talk to.

:::note
While this product is **Gemini Enterprise**, the `googlegenai.VertexAI` struct, `vertexai/modelgarden` and `vertexai/vectorsearch` packages, and `vertexai/` model prefix retain `vertex` naming from the prior brand for backward compatibility.
:::

The Google Generative AI plugin provides access to Google's Gemini models via the Gemini Enterprise API.
...
2. Enable the Gemini Enterprise API (`aiplatform.googleapis.com`) on that project:

   ```shell
   gcloud services enable aiplatform.googleapis.com --project=$PROJECT_ID
   ```

3. Grant the `roles/aiplatform.user` IAM role to the principal your app runs as.
...
The usual setup is a Google Cloud project ID, the [region](https://docs.cloud.google.com/gemini-enterprise-agent-platform/resources/locations) you want to send Gemini Enterprise API requests to, and Application Default Credentials.
...
- By default, `googlegenai.VertexAI` gets the Gemini Enterprise API location from the `GOOGLE_CLOUD_LOCATION` environment variable, then `GOOGLE_CLOUD_REGION`. It accepts a region (`"us-central1"`), a multi-region (`"us"` or `"eu"`), or `"global"`.
...
2. In addition, make sure the account is granted the `roles/aiplatform.user` IAM role. See the Gemini Enterprise [access control](https://docs.cloud.google.com/gemini-enterprise-agent-platform/models/access-control) docs.
...
:::caution
`GEMINI_API_KEY` does **not** enable Express Mode. It names a Gemini Developer API key, which Gemini Enterprise rejects, and a process that also uses the [Google AI plugin](/docs/integrations/google-genai) commonly has one set. Note also that a misconfiguration here panics at `genkit.Init` rather than returning an error.
:::
...
Imagen is not curated in Gemini Enterprise because the service retired it on June 30, 2026. The plugin still resolves `vertexai/imagen-*` and still speaks `generateImages`, but the service no longer serves those models, so such requests now fail. Use the `gemini-*-image` models instead, which generate pictures through `generateContent` on both backends.
...
:::caution
A Model Garden model must be enabled for your project in the [Model Garden console](https://console.cloud.google.com/vertex-ai/model-garden) before a request to it succeeds.
:::
...
1. In addition, make sure the account is granted the `roles/aiplatform.user` IAM role. See the Gemini Enterprise [access control](https://cloud.google.com/vertex-ai/generative-ai/docs/access-control) docs.
...
Important: Gemini Enterprise charges a fee for grounding requests in addition to the cost of making LLM requests. See the [Gemini Enterprise pricing](https://cloud.google.com/vertex-ai/generative-ai/pricing) page and be sure you understand grounding request pricing before you use this feature.
...
Another identifier shipped in the Python SDK registry is `meta/llama-3.2-90b-vision-instruct-maas`. Always confirm the exact model resource name for your project in the [Model Garden](https://cloud.google.com/vertex-ai/generative-ai/docs/model-garden/explore-models) console.

#### Anthropic (Claude) models in Model Garden
...
Gemini Enterprise provides access to various third-party models through Model Garden. Consult the [Model Garden documentation](https://cloud.google.com/vertex-ai/generative-ai/docs/model-garden/explore-models) for the full list of supported models and their capabilities.
