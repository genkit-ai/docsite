...
The Google AI plugin provides access to Google's Gemini models through Vertex AI using Google Cloud authentication.

## Configuration

To use this plugin, add the `genkit_google_genai` package to your project:
...
const response = await ai.generate({
	model: vertexModelGarden.model('meta/llama-4-maverick-17b-128e-instruct-maas'),
