...
`Opts` passes the anthropic-sdk-go client configuration through untouched: retries, timeouts, extra headers, `option.WithMiddleware`, and the SDK's Bedrock and Gemini Enterprise (`vertex`) routing helpers, which are request options too.
