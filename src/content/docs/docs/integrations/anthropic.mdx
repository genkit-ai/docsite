...
```go
import "github.com/genkit-ai/genkit/go/plugins/anthropic"
```
...
You must provide an API key from Anthropic. You can get an API key from the [Anthropic Console](https://console.anthropic.com/). The plugin will automatically use the `ANTHROPIC_API_KEY` environment variable.
...
```go
import (
    "github.com/genkit-ai/genkit/go/plugins/anthropic"
)
