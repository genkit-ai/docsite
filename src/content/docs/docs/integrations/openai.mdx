...
`genkit.Init` panics when neither the `APIKey` field nor `OPENAI_API_KEY` carries a key. There is no base URL environment variable; to reach a proxy or a compatible endpoint, pass `option.WithBaseURL` through `Opts`:
