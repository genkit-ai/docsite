...
The plugin ships a curated per-model catalog, with typed `ModelRef`s exposed via
`OpenAIModels`. Resolution falls back to a dated-suffix alias and then generic
defaults, so uncurated model ids still work with `openAI.model()`.
...
You can reference embedding models using typed `OpenAIEmbedders` references or `openAI.embedder(name)`:
