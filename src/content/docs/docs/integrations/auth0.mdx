...
export const checkCalendarTool = ai.defineTool(
  ...
  async ({ date }) => {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Invalid response from Google Calendar API: ${response.status} - ${await response.text()}`,
      );
    }
...
```javascript
const authorizedTool = auth0AI.withFGA(
  {
...
const authorizer = auth0AI.withFGA({
  buildQuery: async ({ userID, doc }) => ({
    user: userID,
    object: doc,
    relation: 'read',
...
const authorizedTool2 = authorizer(myAITool);
```
...
import { myVectorStoreRetriever } from './my-retriever';

async function main() {
  const user = 'user1';

  // Decorate your Genkit retriever with the FGARetriever
  const secureRetriever = FGARetriever.create({
...
if (interrupt && AuthorizationPendingInterrupt.is(interrupt.metadata)) {
  // do something
  const tool = tools.find((t) => t.name === interrupt.toolRequest.name);

  const restartRequest = tool.restart(
    interrupt,
