...
<Lang lang="go">
  :::note[Feature unavailable for Go] The Auth0 AI plugin is not yet available
  for Go. :::
</Lang>

<Lang lang="python">
  :::note[Feature unavailable for Python] The Auth0 AI plugin is not yet
...
if (interrupt && AuthorizationPendingInterrupt.is(interrupt.metadata)) {
  ...
  const tool = tools.find(
    (t) => t.name === interrupt.metadata.toolCall.toolName,
  );
  tool.restart(
    interrupt,
