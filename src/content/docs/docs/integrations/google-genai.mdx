...
- **googleSearchRetrieval** _{ dynamicRetrievalConfig?: { mode: string, dynamicThreshold: number } } | boolean_
...
- **searchEntryPoint** _{ renderedContent: string }_

  Contains the main search result content formatted for display.

- **groundingSupports** _Array<{ segment: { text: string }, groundingChunkIndices: number[], confidenceScores: number[] }>_
...
- **googleMaps** _{ enableWidget?: boolean }_

  Enables Google Maps grounding.
  Example: `{ enableWidget: true }`

- **toolConfig** _{ googleMaps: { retrievalConfig: { latLng: { latitude: number, longitude: number } } } }?_

  Configuration for tools, such as providing location context for Google Maps.
  Example: `{ googleMaps: { retrievalConfig: { latLng: { latitude: 37.7749, longitude: -122.4194 } } } }`

  > [!TIP]
  > You can improve relevance by providing a location context via `toolConfig`.
...
You can use the `multiSpeakerVoiceConfig` to create conversations with distinct voices for different speakers. This is supported by `gemini-2.5-flash-tts` and `gemini-2.5-pro-tts` models (including preview versions).
...
TTS models automatically detect the input language. Supported languages include:
`en-US`, `fr-FR`, `de-DE`, `es-US`, `ja-JP`, `ko-KR`, `pt-BR`, `zh-CN`, and [more](https://ai.google.dev/gemini-api/docs/speech-generation#languages).
