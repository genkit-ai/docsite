...
const textToVideoFlow = ai.defineFlow('textToVideoFlow', async () => {
...
async function downloadVideo(video: MediaPart, path: string) {
  ...
  Readable.from(videoDownloadResponse.body as any).pipe(fs.createWriteStream(path));
}

await textToVideoFlow();
```
...
- **durationSeconds** _number_ (Veo 2 only)
  ...
  Length of each output video in seconds (5 to 8). Not configurable for Veo 3 (defaults to 8 seconds).

- **enhancePrompt** _boolean_ (Veo 2 only)

  Enable or disable the prompt rewriter. Enabled by default. For Veo 3, the prompt enhancer is always on.
