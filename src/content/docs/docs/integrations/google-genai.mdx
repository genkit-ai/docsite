...
response = await ai.generate(
    prompt=[
        {'text': 'Describe what is in this image'},
        {'media': {'url': f'data:image/jpeg;base64,{image_data}'}},
    ],
)
```
...
response = await ai.generate(
    prompt=[
        ...
        {'media': {'content_type': 'video/mp4', 'url': 'https://example.com/video.mp4'}},
    ],
)
```
...
if operation:
    ...
    if operation.error:
        ...
        print(f'Video URL: {video.media.url}')
```
...
# Extract audio
if response.media:
    audio_data = response.media.url  # Base64 encoded audio
