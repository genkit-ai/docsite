...
Thinking config is available in `GeminiOptions` but specific enum values like `HIGH`/`LOW` might differ. Use `GeminiOptions` to configure.
:::

```dart
final response = await ai.generate(
  model: googleAI.gemini('gemini-3-pro-preview'),
  prompt: 'what is heavier, one kilo of steel or one kilo of feathers',
...
:::note[Feature unavailable for Dart]
Google Maps Grounding is not yet supported in the Dart SDK.
:::
...
:::note[Feature unavailable for Dart]
URL Context is not yet supported in the Dart SDK.
:::
...
// Extract image (inline data URI)
final image = response.media;
...
:::note[Feature unavailable for Dart]
Gemini Files API support is not yet available in the Dart SDK.
:::
...
:::note[Feature unavailable for Dart]
Embeddings are not yet supported in the Dart Google GenAI plugin.
:::
...
:::note[Feature unavailable for Dart]
Image generation via Imagen is not yet supported in the Dart Google GenAI plugin.
:::
...
:::note[Feature unavailable for Dart]
Video generation via Veo is not yet supported in the Dart Google GenAI plugin.
:::
...
- **Embeddings**: `googleAI.embedder()` is not yet implemented.
- **Image Generation**: `imagen-*` models are not yet available.
- **Video Generation**: `veo-*` models are not yet available.
