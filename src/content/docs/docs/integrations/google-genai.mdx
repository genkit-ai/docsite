...
You can reference models in two ways:

- **`ai.WithModelName(name)`**: Resolves a model by its strong identifier (e.g., `"googleai/gemini-2.5-flash"`). This is the recommended way for basic calls.
- **`googlegenai.GoogleAIModelRef(name, config)`**: Creates a static reference that includes a specific configuration.
...
modelRef := googlegenai.GoogleAIModelRef("gemini-2.5-flash", config)
...
modelRef := googlegenai.GoogleAIModelRef("gemini-3-pro-preview", config)
...
modelRef := googlegenai.GoogleAIModelRef("gemini-2.5-pro", config)
...
modelRef := googlegenai.GoogleAIModelRef("gemini-2.5-flash-image", config)
...
- `embedding-001` - Text embedding model (Legacy)
...
modelRef := googlegenai.GoogleAIModelRef("imagen-3.0-generate-002", config)
