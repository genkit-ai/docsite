...
Gemini 2.5 and newer models use an internal thinking process that improves reasoning for complex tasks. You can control the thinking budget and access thought summaries:
...
**Supported Models:**
- All Gemini 2.0 and newer models can process video data
- Gemini 3.0 and 2.5 series models provide the best quality for video understanding
...
- For Gemini 2.5 and newer, you can include up to 10 videos per request
...
Gemini 2.5 and newer models automatically cache common content prefixes, providing a 75% token discount on cached tokens:
...
Models with image generation capabilities can create images. The `gemini-2.5-flash-image` model can generate both images and text in the same response:

```typescript
const response = await ai.generate({
  model: googleAI.model('gemini-2.5-flash-image'),
  prompt: 'Create a picture of a futuristic city at sunset and describe it',
  config: {
...
The Google GenAI plugin provides access to image generation through Imagen models.

#### Basic Usage

To generate an image using an Imagen model:

```ts
const response = await ai.generate({
  model: googleAI.model('imagen-4.0-generate-001'),
  prompt: 'A futuristic city with towering skyscrapers and flying cars',
  config: {
    numberOfImages: 4,
    aspectRatio: '16:9',
    enhancePrompt: true,
...
#### Imagen Configuration Options

The Imagen models support various configuration options:

- `numberOfImages`: Number of images to generate (1-4). Default is 4.
- `aspectRatio`: Aspect ratio of the generated image.
  ...
  - `"16:9"`: Widescreen
- `personGeneration`: Allow the model to generate images of people.
  - `"dont_allow"`: Block generation of people
  - `"allow_adult"`: Allow generation of adults but not children
  - `"allow_all"`: Allow adults and children (supported on specific models)
- `safetySetting`: Safety filter level.
  - `"block_low_and_above"`: Block low probability and above
  - `"block_medium_and_above"`: Block medium probability and above
  - `"block_only_high"`: Block only high probability
- `negativePrompt`: Text description of what to exclude from the image.
- `addWatermark`: boolean. Whether to add invisible watermark (SynthID). Default is true.
- `enhancePrompt`: boolean. Whether to use prompt rewriting for better quality. Default is true.
