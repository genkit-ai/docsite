...
export const audioGeneratorFlow = ai.defineFlow(
  {
    name: 'audioGeneratorFlow',
  },
  async () => {
    ...
    if (response.media?.url) {
      const pcmData = Buffer.from(response.media.url.substring(response.media.url.indexOf(',') + 1), 'base64');
...
audioGeneratorFlow().catch(console.error);
```
...
For more details on speech generation and processing, see the [official Gemini API documentation](https://ai.google.dev/gemini-api/docs/speech-generation).
