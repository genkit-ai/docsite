...
The Gemini API guarantees that the model output will conform to a specified JSON schema, but it relies on a specific subset of the OpenAPI 3.0 standard. When defining Zod schemas for structured output (e.g., using `z.object` in `output.schema`), keep the following limitations in mind:
