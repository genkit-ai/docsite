...
| `HTTPClient` | `*http.Client` | Used verbatim when set. The plugin does not install its default OpenTelemetry instrumented transport, so wrap your own transport with `otelhttp` if you want tracing. |
...
:::note
Vertex AI spells three of these differently: the omni model, the Gemini 2.5 TTS pair, and Veo 3.1. See the [Vertex AI plugin](/docs/integrations/vertex-ai) for that backend's IDs. The [Gemini API models documentation](https://ai.google.dev/gemini-api/docs/models) is the authority on what the service currently serves.
:::
...
:::note
The catalog is shared with the Vertex AI plugin. The Gemini API itself serves only the `gemini-embedding-*` models, so `text-embedding-005`, `text-embedding-004`, `text-multilingual-embedding-002`, and `multimodalembedding` resolve here but fail at the service. Use them through the [Vertex AI plugin](/docs/integrations/vertex-ai).
:::
...
**Gemini 3 Series** - Latest models:

- `gemini-3.7-flash` - State-of-the-art multimodal reasoning with hybrid thinking support
- `gemini-3.6-flash` - Fast and intelligent for high-volume production workloads
