...
- **`googleSearchRetrieval`** (`{ dynamicRetrievalConfig?: { mode: string, dynamicThreshold: number } } | boolean`)
...
- **`groundingSupports`** (`Array<{ segment: { text: string }, groundingChunkIndices: number[], confidenceScores: number[] }>`)
...
- **`toolConfig`** (`{ googleMaps: { retrievalConfig: { latLng: { latitude: number, longitude: number } } } }`?)
