...
2. Create a Firestore Database and a Collection within that database to store the documents that will be indexed. More information to create Firestore databases is available [here](https://firebase.google.com/docs/firestore/quickstart#create)

## Configuration

To use Vertex AI Vector Search with Firestore, use the `define_vertex_vector_search_firestore()` helper function:
```python
...
retriever_name = define_vertex_vector_search_firestore(
    ai,
    name='my_vertex_firestore_retriever',
...
- **firestore_client**: A `google.cloud.firestore.AsyncClient` object for database queries.
- **collection_name** (str): The name of the Firestore collection containing documents.
...
To populate your Firestore collection with documents and vector search index:
```python
from genkit import Genkit
from genkit.plugins.google_genai import VertexAI
from google.cloud import firestore
...
async def index_documents_to_vertex_search(documents: list[str], collection: str):
    """Index documents in Firestore with Vertex AI Vector Search.
    ...
    for i, text in enumerate(documents):
        ...
        # Create unique document ID for vector search
        doc_id = f'doc-{i}'
...
# ]
# import asyncio
# asyncio.run(index_documents_to_vertex_search(documents, 'documents'))
```
...
async def vector_search_flow(query: str) -> str:
    """Search using Vertex AI Vector Search with Firestore.
    ...
    results = await ai.retrieve(
        retriever='my_vertex_firestore_retriever',
        query=query,
        options={'limit': 5},
    )
