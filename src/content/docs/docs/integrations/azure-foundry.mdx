...
const response = await ai.generate({
	model: gpt5,
	prompt: [{ text: 'What animal is in the photo?' }, { media: { url: imageUrl } }],
	config: {
...
Azure OpenAI models support tool calling for function execution:
...
```typescript
const { response, messages } = await ai.generate({
	messages: [
