...
If you have set the `AWS_` environment variables (AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, AWS_REGION), you can omit the region parameter:
...
You can use AWS Bedrock models within Genkit flows:
...
AWS Bedrock models support tool calling for function execution:
...
The AWS Bedrock plugin allows you to define models with custom configurations that are not defined in the plugin. You can also specify model parameters like temperature and max output tokens.
