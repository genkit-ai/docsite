...
This Genkit plugin allows you to use [AWS Bedrock](https://aws.amazon.com/bedrock/) through their official APIs. AWS Bedrock is a fully managed service that provides access to foundation models from leading AI companies through a single API. The plugin enables you to use these models for text generation, embeddings, reranking, and image generation. It supports features like tool calling, streaming, multimodal inputs, prompt caching, and cross-region inference for improved performance and resiliency.

## Installation

Install the plugin in your project with `uv` or `pip`:
...
- **Text Generation**: Support for multiple foundation models (Amazon Nova, Anthropic Claude, Meta Llama, Mistral, Cohere) via the Bedrock Converse and ConverseStream APIs
- **Embeddings**: Support for text and multimodal embedding models from Amazon Titan and Cohere
- **Image Generation**: Support for image generation models like Amazon Titan Image Generator and Stability AI (SDXL)
- **Reranking**: High-precision retrieval reranking with Cohere Rerank
- **Prompt Caching**: Native Bedrock prompt cache checkpoints via `cache_point_part()`
- **Streaming**: Full streaming support for real-time responses
- **Tool Calling**: Complete function calling capabilities with Pydantic schema validation
- **Multimodal Support**: Support for text, image, and document (PDF/DOCX/CSV) inputs
- **Cross-Region Inference**: Support for inference profiles to improve throughput and resiliency
...
# Initialize Genkit with the Bedrock plugin
ai = Genkit(
    plugins=[Bedrock(region='us-east-1')],
...
You can configure the plugin by passing options to `Bedrock()`:

```python
from genkit import Genkit
from genkit_amazon_bedrock import Bedrock

# Uses ambient AWS credentials and default region from environment
ai = Genkit(plugins=[Bedrock()])
```

### Passing Explicit Region and Credentials

For custom configuration, pass explicit region or credentials:
...
The plugin uses the standard AWS SDK credential resolution chain.
...
2. **AWS Credentials File** (`~/.aws/credentials` and `~/.aws/config`):
```ini
[default]
aws_access_key_id = your-access-key
aws_secret_access_key = your-secret-key
...
3. **IAM Roles**: When running on AWS compute services (EC2, ECS, Lambda) or Google Cloud Run via workload federation.

4. **AWS SSO / CLI**: Authenticate with `aws sso login`.

### Required IAM Permissions

Create an IAM policy with permissions for model invocation:
...
Cross-region inference profiles (e.g. `us.anthropic.claude-sonnet-4-5-20250929-v1:0`) use inference profile ARNs, so ensure `arn:aws:bedrock:*:*:inference-profile/*` is included in your policy.

### Model Access

Model access is managed per AWS account and region in the AWS Bedrock Console under **Model access**. Anthropic Claude models additionally require accepting the one-time Anthropic use-case agreement in the console before first invocation.

## Using Models & Inference Profiles

Any routable Bedrock model ID or cross-region inference profile resolves dynamically on demand using the `bedrock/<model-id>` namespace:
...
- **Meta Llama 3.3 70B**: `bedrock/us.meta.llama3-3-70b-instruct-v1:0`
- **DeepSeek R1**: `bedrock/us.deepseek.r1-v1:0`

```python
response = await ai.generate(
    model='bedrock/us.anthropic.claude-sonnet-4-5-20250929-v1:0',
    prompt='Explain declarative AI workflows in two sentences.',
)
```
...
response = await ai.generate(
    prompt='Analyze this review: "The battery lasts all day and the camera is sharp, but the charger was missing."',
    output_format='json',
    output_schema=ProductReview,
    output_instructions=True,
)
print(response.output)  # ProductReview instance
```
...
class OrderStatusInput(BaseModel):
    order_id: str = Field(description='The order tracking ID')


@ai.tool(description='Look up shipping status for an order')
async def get_order_status(input: OrderStatusInput) -> str:
    return f'Order {input.order_id} is in transit, arriving tomorrow by 5 PM.'


response = await ai.generate(
    prompt='Check the status of order ORD-98721, then answer in one sentence.',
...
@ai.flow()
async def story_stream(topic: str, ctx: ActionRunContext) -> str:
    stream_response = ai.generate_stream(
        prompt=f'Write a short inspiring story about {topic}.',
    )
    async for chunk in stream_response.stream:
        ctx.send_chunk(chunk.text)
    return (await stream_response.response).text
...
flow_response = story_stream.stream('space exploration')
async for text in flow_response.stream:
    print(text, end='', flush=True)
result = await flow_response.response
```
...
response = await ai.generate(
    prompt=[
        Part(root=MediaPart(media=Media(url=image_data_url))),
        Part(root=TextPart(text='Extract the total amount and due date from this invoice.')),
    ],
)
print(response.text)
```
...
Prompt caching helps save input token costs and reduce latency for repeated contexts:

```python
from genkit import Part, TextPart
from genkit_amazon_bedrock import cache_point_part

# The cache point goes AFTER the reusable static context
system = [
    Part(root=TextPart(text=LONG_DOCUMENTATION_TEXT)),
    cache_point_part(),
]

# Calls sharing the byte-identical prefix will hit the cache
res1 = await ai.generate(system=system, prompt='What is the refund policy?')
res2 = await ai.generate(system=system, prompt='How do I cancel my plan?')

print(res2.usage.cached_content_tokens)
```

## Additional Capabilities

- **Embedders**: Pass embedder IDs in `Bedrock(embedders=['amazon.titan-embed-text-v2:0'])` and call `ai.embed()`.
- **Image Generation**: Register image models with `ModelDefinition(name='stability.sd3-5-large-v1:0', type='image')` or `amazon.titan-image-generator-v2:0`.
- **Reranking**: Call `rerank()` directly on the `Bedrock` plugin instance with `BedrockRerankOptions` for high-precision retrieval re-ranking.
