...
The `genkit-amazon-bedrock` package provides the `Bedrock` plugin for accessing foundation models, embedders, rerankers, and image generation models hosted on [AWS Bedrock](https://aws.amazon.com/bedrock/).

The plugin supports:

- **Language Models**: Converse and ConverseStream APIs across Anthropic Claude on Bedrock, Amazon Nova, Amazon Titan, Mistral, and Cohere.
- **Embeddings**: Text and multimodal embeddings with Amazon Titan Embeddings (v1/v2) and Cohere Embed.
- **Reranking**: High-precision retrieval reranking with Cohere Rerank.
- **Image Generation**: Amazon Titan Image Generator and Stability AI (SDXL).
...
Initialize the plugin in Genkit:

```python
from genkit import Genkit
from genkit_amazon_bedrock import Bedrock

# 1. Initialize Bedrock (uses ambient AWS credentials and AWS_REGION)
ai = Genkit(
    plugins=[Bedrock()],
...
You can optionally pass an explicit region, credentials, or custom boto3 session:
...
Model access is granted per AWS account and per region in the AWS Bedrock Console under **Model access**. Most foundation models (such as Anthropic Claude) require requesting access and accepting the one-time Anthropic use-case agreement in the console before they can be invoked.

### IAM Permissions

The minimal IAM policy covering model invocation and streaming:
...
Cross-region inference profiles (such as `us.anthropic.claude-sonnet-4-5-20250929-v1:0`) use inference-profile ARNs, so ensure the policy includes `arn:aws:bedrock:*:*:inference-profile/*`.

### Credentials

Credentials resolve through standard AWS SDK chains:
- Environment variables (`AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_SESSION_TOKEN`, `AWS_REGION`)
- Shared config and credentials files under `~/.aws` (configurable with `AWS_PROFILE`)
- IAM roles attached to EC2, ECS, Cloud Run, or AWS Lambda
- SSO profiles (`aws sso login`)
...
response = await ai.generate(
    prompt='Summarize the key benefits of declarative AI orchestration in three bullets.',
)
print(response.text)
```
...
response = await ai.generate(
    prompt='Check the status of order ORD-98721, then answer in one friendly sentence.',
...
Bedrock prompt caching saves input token costs and latency by caching reusable prompt prefixes:

```python
from genkit import Part, TextPart
from genkit_amazon_bedrock import cache_point_part

# 1. Place the cache point AFTER the long static content
system = [
    Part(root=TextPart(text=LONG_DOCUMENTATION_TEXT)),
    cache_point_part(),
]

# 2. Subsequent calls with the identical prefix hit the cache
...
- **Reranking**: Use `BedrockRerankOptions` and call `rerank()` directly on the `Bedrock` plugin instance for high-precision retrieval re-ranking.
