...
Credentials never cross providers. The OpenAI SDK reads `OPENAI_API_KEY`, `OPENAI_ORG_ID`, and `OPENAI_PROJECT_ID` from the environment for every client it builds, and the base plugin clears all three before applying what a provider plugin composes, so a request to DeepSeek or xAI carries that provider's key and nothing of OpenAI's. Only the `openai` plugin reads the organization and project variables, and it sets them explicitly.
