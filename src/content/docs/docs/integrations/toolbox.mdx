...
export const customerAnalyticsFlow = ai.defineFlow(
  ...
  async ({ query }) => {
    ...
    // Extract the tool output from the conversation history
    const toolMessage = response.messages.find((m) => m.role === 'tool');
    const toolData = toolMessage?.content.find((p) => !!p.toolResponse)
      ?.toolResponse?.output;
...
- See [authorization patterns](/guides/deployment/authorization) for securing your tools
