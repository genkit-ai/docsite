...
In addition to generating text and images, Genkit can also be used to generate audio content through text-to-speech (TTS) models. This capability allows you to convert text into natural-sounding speech for various applications.

To generate audio using Genkit, you can use the `generate()` method with a TTS-capable model and specify the appropriate configuration:
...
Genkit also supports generating audio with multiple speakers, each with their own voice:
...
const response = await ai.generate({
  ...
  prompt: `Here's the dialog:
    ...
    Speaker2: "I agree, it makes working with AI models so much easier."`,
});

// Process the audio data as shown in the previous example
```

When using multi-speaker configuration, the model will automatically detect speaker labels in the text (like "Speaker1:" and "Speaker2:") and apply the corresponding voice to each speaker's lines.

The specific configuration options available will depend on the model and provider being used. Refer to the model provider's documentation for details on available voices and configuration options.
