...
example, to generate an image using the Imagen model through Vertex AI:
...
Note that the example above includes Gemini-specific configurations. The specific configuration options available will depend on the model and provider being used. Refer to the model provider's documentation for details on available voices and configuration options.
