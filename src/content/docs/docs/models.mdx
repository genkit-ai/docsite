...
Some models can generate media in addition to text. For example, you can use image generation models like Imagen to create images from text prompts, or video generation models like Veo.

The Google GenAI plugin provides access to Imagen models for image generation:
...
# Access the generated media
if response.media:
    for media_item in response.media:
        print(f"Media type: {media_item.content_type}")
        print(f"Media URL: {media_item.url}")
```

To save the generated image to a file:
...
if response.media:
    ...
    # Media URL is a data URI, extract the base64 content
    if media.url.startswith('data:'):
        # Parse data URI format: data:image/png;base64,<content>
...
:::note[Media generation models]
Media generation capabilities depend on the model and plugin you're using. Check the documentation for your specific model plugin to see which media generation models are available and their supported features.
:::
