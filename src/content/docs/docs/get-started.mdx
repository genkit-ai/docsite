...
Update your `package.json` to use ES modules (required for Genkit and modern TypeScript):
...
:::note
The `"type": "module"` setting enables ES module syntax (`import`/`export`) which is required for Genkit packages and modern TypeScript configurations. This is the recommended approach for new Node.js projects.
:::

This sets up your project structure with ES module support and a TypeScript entry point at `src/index.ts`.
