...
Context must be a dictionary, but its properties are yours to decide.
...
To use context within an action, you can access the context helper
supplied to your function or query `ai.current_context()`:
...
@ai.flow(name='summarizeMessages')
async def summarize_history(input: SummarizeInput, ctx: ActionRunContext) -> str:
    auth = ctx.context.get('auth') if ctx.context else None
    if not auth or 'uid' not in auth:
        raise ValueError('Must supply auth context.')
    
    messages = await list_messages_between(input.friend_uid, auth['uid'])
...
@ai.tool(description="search the current user's notes for info")
async def search_notes(input: SearchInput) -> list[Note]:
    context = ai.current_context() or {}
    auth = context.get('auth')
    if not auth or 'uid' not in auth:
        raise ValueError('Must be called by a signed-in user.')
    return await search_user_notes(auth['uid'], input.query)
```
...
```python
summary = await summarize_history.run(
    SummarizeInput(friend_uid=friend.uid),
...
```python
hello_prompt = ai.prompt('sayHello')
await hello_prompt(
    input={'pirateStyle': True},
    context={'auth': current_user},
)
```
...
```python
@ai.flow()
async def my_flow(input_val, ctx: ActionRunContext):
    # nested calls inherit context automatically
    await other_flow(input_val)

    # override the existing context completely — pass context via .run()
    await other_flow.run(
        input_val,
        context={"newContext": True},
    )

    # or selectively override
    current_ctx = ctx.context or {}
    await other_flow.run(
        input_val,
        context={**current_ctx, "updatedContext": True},
    )
```

When context is replaced, it propagates the same way. In this example,
any actions that `other_flow` called during its execution would inherit the
overridden context.
