...
JavaScript provides a comprehensive `context` object that can propagate generation and execution context throughout the process. This context is made available to all actions including [flows](/docs/flows), [tools](/docs/tool-calling), and [prompts](/docs/dotprompt).
...
- Learn about [creating flows](/docs/flows) to build workflows that use context
- Explore [tool calling](/docs/tool-calling) to understand how context propagates to tools
- See [developer tools](/docs/devtools) for debugging context-aware applications
- Check out [generating content](/docs/models) for using context in generation calls
