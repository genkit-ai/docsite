...
You can call your deployed flows using the Genkit client library. This library provides functions for both non-streaming and streaming flow calls.

Learn about flows in "[Defining AI workflows](/docs/flows/?lang=js)".
...
You can call your deployed flows using the Genkit Dart client library. This library is compatible with any Dart environment, including server-side applications and Flutter. It provides a type-safe way to interact with both non-streaming and streaming flows.

Learn about flows in "[Defining AI workflows](/docs/flows/?lang=js)".

Terminology: You will see the term "action" being used. Genkit's core framework is built on the "action" primitive, which enables observability/tracing, streaming and Dev UI interation. In theory, any action can be made remotely accessible with Genkit, so client is not limited to flows, but any action that the server makes available.

### Non-streaming Flow Calls

`defineRemoteAction` function lets you create a definition of the remote action. It's used for both non-streaming and streaming actions.

For a non-streaming response, call the action `await` the action. This is suitable for flows that return a single, complete output.
...
Detailed documentation can be found at https://firebase.google.com/docs/functions/callable?gen=2nd#dart
