...
on them. That includes the model provider plugins, which use plain HTTP clients:
a provider call shows up as Genkit's model span with nothing beneath it. To see
the HTTP request as well, hand the plugin a client whose transport is wrapped
with `otelhttp.NewTransport`, through its `HTTPClient` field where it has one.
