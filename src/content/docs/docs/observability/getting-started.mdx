...
<LanguageContent lang="python">
Install the `genkit-plugin-firebase` plugin in your project:

```bash
uv add "genkit-plugin-firebase[telemetry]"
```
</LanguageContent>
...
Import `add_firebase_telemetry` into your Genkit configuration file (the
file where `genkit(...)` is initalized), and call it:
...
<LanguageContent lang="python">
1. In your Genkit code, use `add_gcp_telemetry` with `force_dev_export=True` to send telemetry from
   your local environment.
