...
```bash
genkit eval:extractData qaFlow --label firstRunSimple --output factsEvalDataset.json -- <command to start your app>
```
...
```bash
genkit eval:extractData qaFlow --maxRows 2 --output factsEvalDataset.json -- go run main.go
```
...
```bash
genkit eval:extractData qa_flow --label firstRunSimple --output factsEvalDataset.json -- uv run main.py
```
