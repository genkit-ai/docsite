...
![Create dataset dialog in Genkit Developer UI](../../../assets/devui-dataset-create.png)
...
![Dataset examples table in Genkit Developer UI](../../../assets/devui-dataset-examples.png)
...
![Configure and run evaluation in Genkit Developer UI](../../../assets/devui-eval-run.png)
...
![Evaluation run results in Genkit Developer UI](../../../assets/devui-eval-results.png)
...
Genkit supports additional evaluators through plugins, like the Gemini Enterprise Rapid Evaluators, which you can access via the [Gemini Enterprise plugin](/docs/integrations/vertex-ai#evaluation-metrics).
...
genkit eval:flow qaFlow --input myFactsQaDataset -- <command to start your app>

# or, using a dataset from a file
genkit eval:flow qaFlow --input testInputs.json -- <command to start your app>
```
...
```bash
genkit eval:flow qaFlow --input testInputs.json --context '{"auth": {"email_verified": true}}' -- <command to start your app>
```
...
```bash
genkit eval:flow qaFlow --input testInputs.json --evaluators=genkitEval/maliciousness,genkitEval/answer_relevancy -- <command to start your app>
```
...
```bash
genkit flow:batchRun qaFlow testInputs.json --label firstRunSimple -- <command to start your app>
```
...
[Gemini Enterprise evaluation metrics](/docs/integrations/vertex-ai#evaluation-metrics)
...
```bash
# Referencing an existing dataset
genkit eval:flow qa_flow --input myFactsQaDataset -- uv run main.py

# or, using a dataset from a file
genkit eval:flow qa_flow --input testInputs.json -- uv run main.py
```
...
```bash
genkit eval:flow qa_flow --input testInputs.json --context '{"auth": {"email_verified": true}}' -- uv run main.py
```
...
```bash
genkit flow:batchRun qa_flow testInputs.json --label firstRunSimple -- uv run main.py
```
