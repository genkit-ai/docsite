...
def ask_question(input: QuestionInput, ctx: ToolRunContext) -> str:
    ...
    # This line is never reached due to interrupt
    return ''
```
...
if response.interrupts and len(response.interrupts) > 0:
...
async def interactive_session():
    ...
    while response.interrupts and len(response.interrupts) > 0:
        answers = []
