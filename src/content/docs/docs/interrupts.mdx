...
<div style="display: flex; justify-content: space-between; align-items: center; gap: 1rem; margin: 1rem 0 1rem 0;">
  <LanguageSelector supportedLanguages="go js python" />
  <CopyMarkdownButton />
</div>
...
for _, part := range response.Message.Content {
	if part.IsToolRequest() {
		if _, ok := part.Metadata["interrupt"]; ok {
			interruptCount++
		}
	}
}
	...
	for _, part := range response.Message.Content {
		if part.IsToolRequest() {
			if _, ok := part.Metadata["interrupt"]; ok {
				...
				answers = append(answers, askQuestion.Respond(
					part,
					// send the tool request input to the user to respond
					askUser(part.ToolRequest.Input),
					nil,
				))
			}
		}
