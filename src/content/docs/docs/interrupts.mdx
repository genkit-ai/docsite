...
```python
from genkit import FinishReason, Genkit, Part, tool_response, ToolRunContext
...
with `Part(root=interrupt_part)` for each interrupted request:

```python
# Get the user's answer (e.g., from user input)
user_answer = 'b'  # User selected option b

interrupt_part = response.interrupts[0]

# Resume generation with the tool response
response = await ai.generate(
    messages=response.messages,
    tool_responses=[tool_response(Part(root=interrupt_part), user_answer)],
    tools=['ask_question'],
)
```
...
```python
from genkit import Part, tool_response
...
async def interactive_session():
    ...
    while response.interrupts:
        ...
        for interrupt in response.interrupts:
            ...
            answers.append(tool_response(Part(root=interrupt), user_input))
