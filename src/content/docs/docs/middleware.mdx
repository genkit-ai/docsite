...
Grants the model access to a single root directory by injecting standard file manipulation tools (`list_files`, `read_file`, plus `write_file` and `edit_file` when writes are enabled). Path safety is enforced by `os.Root` (Go 1.25+), which rejects any path that resolves outside the root, including via `..`, absolute paths, or symbolic links.
