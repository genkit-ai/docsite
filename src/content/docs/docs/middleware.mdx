...
The built-in middleware implement `ai.Middleware` on pointer receivers, so they are passed as `&middleware.Retry{...}`. A custom type with value receivers is passed as a bare literal.
...
- `Statuses` (optional): A list of `status.Name` values, from `github.com/firebase/genkit/go/core/status`, that should trigger a retry (default: `status.Unavailable`, `status.DeadlineExceeded`, `status.ResourceExhausted`, `status.Aborted`, `status.Internal`).
- `InitialDelayMs` (optional): The initial delay between retries in milliseconds (default: 1000).
...
Note the receivers. `Timing` declares `Name` and `New` on value receivers, so a bare `Timing{...}` satisfies `ai.Middleware`. The built-in middleware declare theirs on pointer receivers, which is why they are always passed as `&middleware.Retry{...}`.
