...
class MyPlugin(Plugin):
    ...
    def __init__(self, api_key: str | None = None):
        ...
        """
        self.api_key = api_key or os.environ.get('MY_PLUGIN_API_KEY')
        if not self.api_key:
            raise ValueError(
...
class MyModelPlugin(Plugin):
    ...
    def _create_model_action(self, name: str) -> Action:
        ...
        async def generate(
        ...
        ) -> AsyncIterator[ModelResponseChunk] | ModelResponse:
            ...
            Returns:
                Either a complete ModelResponse or an async iterator of chunks
...
The `ModelCapabilities` object declares what features the model supports:
...
| `system_role` | Supports system messages |
